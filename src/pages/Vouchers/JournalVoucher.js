...
const JournalVoucher = ({navigation}) => {
  ...
  const handleSubmit = async (isOptional = false) => {
    ...
    setSubmitting(true);
    try {
      const result = await tallyWriteAPI.createJournal({
  ...
  return (
    <View style={commonScreenStyles.container}>
      <Header title="Journal Voucher" leftIcon="chevron-left" onLeftPress={() => navigation.goBack()} />
      <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false}
        contentContainerStyle={[commonScreenStyles.scrollContent, isKeyboardVisible && styles.keyboardPadding]}
        keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive">
        <VoucherInfo />
        <JournalVoucherForm ref={formRef} scrollViewRef={scrollViewRef} isKeyboardVisible={isKeyboardVisible} />
        <View style={commonScreenStyles.bottomSpacer} />
      </ScrollView>
      <BottomArea
        buttonText={submitting ? 'Submitting...' : 'Submit Journal'}
