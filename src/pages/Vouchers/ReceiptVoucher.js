...
const ReceiptVoucher = ({navigation}) => {
  ...
  const handleSubmit = async (isOptional = false) => {
    ...
    setSubmitting(true);
    try {
      const result = await tallyWriteAPI.createReceipt({
      ...
      if (result?.status) {
        ...
        Alert.alert(
          isOptional ? 'Optional Entry Saved' : 'Receipt Created in Tally',
  ...
  return (
    <View style={commonScreenStyles.container}>
      <Header title="Receipt Voucher" leftIcon="chevron-left" onLeftPress={() => navigation.goBack()} />
      <ScrollView ref={scrollViewRef} showsVerticalScrollIndicator={false}
        contentContainerStyle={[commonScreenStyles.scrollContent, isKeyboardVisible && styles.keyboardPadding]}
        keyboardShouldPersistTaps="handled" keyboardDismissMode="interactive">
        <VoucherInfo />
        <ReceiptVoucherForm ref={formRef} scrollViewRef={scrollViewRef} isKeyboardVisible={isKeyboardVisible} />
        <View style={commonScreenStyles.bottomSpacer} />
      </ScrollView>
      <BottomArea
        buttonText={submitting ? 'Submitting...' : 'Submit Receipt'}
