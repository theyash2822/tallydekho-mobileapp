...
const DebitNoteForm = ({navigation}) => {
  ...
  const handleSubmit = async (isOptional = false) => {
    ...
    setSubmitting(true);
    try {
      const result = await tallyWriteAPI.createDebitNote({
        companyGuid: selectedCompany.guid || selectedCompany.id,
        companyName: selectedCompany.name,
        date: new Date().toISOString().slice(0,10).replace(/-/g,''),
        partyLedger, amount: parseFloat(amount) || 0,
        narration: reason, reference: originalRef, isOptional,
      });
      if (result?.status) {
        const num = result?.voucherNumber || '';
        Alert.alert('Debit Note Created!', `${num ? `Voucher: ${num}\n` : ''}₹${parseFloat(amount).toLocaleString('en-IN')}`, [{text: 'OK', onPress: () => navigation.goBack()}]);
  ...
  return (
    <View style={commonScreenStyles.container}>
      <Header title="Debit Note" leftIcon="chevron-left" onLeftPress={() => navigation.goBack()} />
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[commonScreenStyles.scrollContent, {padding: 16}]}>
        <Text style={styles.label}>Party / Vendor *</Text>
        ...
        <TextInput style={[styles.input, {height: 80, textAlignVertical: 'top'}]} value={reason} onChangeText={setReason} placeholder="Reason for debit note" multiline placeholderTextColor="#9CA3AF" />
        <View style={commonScreenStyles.bottomSpacer} />
      </ScrollView>
      <BottomArea buttonText={submitting ? 'Submitting...' : 'Create Debit Note'} showSecondButton={true} secondButtonText="Save as Optional" onPress={() => !submitting && handleSubmit(false)} onSecondPress={() => !submitting && handleSubmit(true)} />
    </View>
  );
};
