...
const AddOrderSection = ({
  ...
  showBarcodeBelow ,// 👈 add default value
  showBarcodeAbove ,// 👈 add default value
  label1,
  initialProducts = [],
  products,
  setProducts,
  ...
  const [localStocks, setLocalStocks] = useState(initialProducts);
  ...
  // Update stocks when initialProducts change (e.g., when navigating with params)
  useEffect(() => {
    if (initialProducts && initialProducts.length > 0 && products === undefined) {
      setLocalStocks(initialProducts);
    }
  ...
  return (
    <View style={styles.container}>
      ...
      <AddProductModal
        ...
        showBarcodeBelow={showBarcodeBelow} 
        showBarcodeAbove={showBarcodeAbove}
